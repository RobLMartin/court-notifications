...
function Home2({ state, dispatch }) {
  ...
  if (state.selectedDefendant) {
    let defendantName = state.cases.filter(
      (item) => item.defendant + "." + item.dob === state.selectedDefendant
    )[0].defendant;
    headerText = `${t("signup.title")} ${defendantName}`;
    ...
    <div>
      ...
      <ol className="usa-process-list">
        <li className="usa-process-list__item">
          ...
          <p className="margin-top-05">{t("search.description")}</p>
